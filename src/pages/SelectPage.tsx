...
function ReactFlowPro({ strength = -300, distance = 300 }: ExampleProps = {}) {
  ...
  useForceLayout({ strength, distance, times, setIsLoading })
    ...
    (_, node) => {
      const newNodeList = nodes.map((_node) => {
        if (_node.id === node.id) {
          ...
          if (newNode.id.includes('level0')) {
            newNode.className = styles.node1_center
          } else if (newNode.id.includes('level0')) {
            newNode.className = styles.node1_level0_node
          } else if (newNode.id.includes('level1')) {
            newNode.className = styles.node1_level1_node
          } else if (newNode.id.includes('level2')) {
  ...
  return (
    ...
    >
      {isLoading && (<>
        ...
        <Panel className="transform -translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2 z-50">
